...
%% This is *the* estone benchmark, it tries to meassure
%% the performance of an erlang system in a way which
%% makes it possible to compare different implementations
%% of erlang as well as different HW.
%% estone consist of a number of micro benchmarks that
%% that can be run independently. If all the microbenchmarks
%% are combined, we get a system where erlang 4.3.1 runs with
%% 1000 estones on a 85 Mhz sparc 5.
...
pp(Out, Ms) ->
    {Total, Stones} = sum_millis(Ms, 0, 0),
    io:format(Out, "**** Total time ~w seconds ****~n", [Total / 1000]),
    io:format(Out, "**** ESTONES = ~w~n~n", [Stones]),
    io:format(Out, "~-31s      ~-12s  ~-10s %    ~-8s ~n~n",
...
determine_loop_data(I, Ms) ->
    ...
    Ms2 = case sum_micros(Ms) of
		  ...
		  set_loops(Tmp);
	      Other ->
		  io:format("Micros don't sum up ~w~n", [Other]),
		  exit(badsum)
	  end,
...
ppld([M| Tail], Mss) ->
    Str = M#micro.str,
    Loops = find_loops(M#micro.function, Mss),
    io:format("~-20s  Adjusted Mean: ~-5w   ~w~n",
	      [Str, mean(Loops), Loops]),
    ppld(Tail, Mss).
