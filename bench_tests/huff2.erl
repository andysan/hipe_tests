...
native_compile() ->
   Opts = [o3],
   ix:compile({?MODULE, test, 0}, Opts),
   ix:compile({?MODULE, get_data, 1}, Opts),
   ix:compile({?MODULE, pack_unpack, 1}, Opts),
   ix:compile({?MODULE, unpack, 3}, Opts),
   ix:compile({?MODULE, find_byte, 2}, Opts),
   ix:compile({?MODULE, bytes_to_bits, 1}, Opts),
   ix:compile({?MODULE, bits_to_bytes, 1}, Opts),
   ix:compile({?MODULE, pack_data, 2}, Opts),
   ix:compile({?MODULE, get_code, 2}, Opts),
   ix:compile({?MODULE, make_codes, 2}, Opts),
   ix:compile({?MODULE, make_codes, 3}, Opts),
   ix:compile({?MODULE, build_code_tree, 1}, Opts),
   ix:compile({?MODULE, sort_trees, 2}, Opts),
   ix:compile({?MODULE, insert_tree, 2}, Opts),
   ix:compile({?MODULE, build_freq_trees, 1}, Opts),
   ix:compile({?MODULE, build_freq_table, 2}, Opts),
   ix:compile({?MODULE, occurs, 3}, Opts),
   ix:compile({?MODULE, reverse, 1}, Opts),
   ix:compile({?MODULE, reverse, 2}, Opts),
   ix:compile({?MODULE, append, 2}, Opts).
