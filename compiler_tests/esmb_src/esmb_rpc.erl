...
do_samr_test(S, Pdu, IpStr) ->
    ...
    {Rid,_Type} = rpc_samr_lookup_names(S, Pdu, IpStr, CtxHandle3, Name),
...
%%% @end
%%%
rpc_samr_open_domain(S, Pdu, _IpStr, CtxHandle, Domain) ->
...
%%% @end
%%%
rpc_samr_lookup_names(S, Pdu, _IpStr, CtxHandle, Names) ->
...
%%% @end
%%%
rpc_samr_open_user(S, Pdu, _IpStr, CtxHandle, User) ->
...
%%% @end
%%%
rpc_samr_user_groups(S, Pdu, _IpStr, CtxHandle) ->
...
%%% @end
%%%
rpc_samr_lookup_rids(S, Pdu, _IpStr, CtxHandle, Ns0) ->
...
e_samr_enum_doms(_UnicodeP, CtxHandle) ->
    ResumeHandle = 0,         % ?
...
parse_user_groups(Count, <<Rid:32/little,_RidAttr:32/little,Rest/binary>>) 
  when Count > 1 ->
    [#name_entry{rid = Rid} | 
     parse_user_groups(Count - 1, Rest)];
parse_user_groups(1, <<Rid:32/little,_RidAttr:32/little>>) ->
    [#name_entry{rid = Rid}];
...
parse_edom_bodies(_UnicodeP, _Blob, []) ->
    [];
...
parse_nse_bodies(_UnicodeP, _Blob, []) ->
    [];
parse_nse_bodies(UnicodeP, Blob, [H|T]) ->
    <<_MaxCount:32/little,
      ...
      ActualCount:32/little,
      B0/binary>> = Blob,
    {Share,_B1} = str_extract(UnicodeP, ActualCount, B0),
...
d_rpc_response(<<?RPC_VERSION, ?RPC_MINOR_VERSION, 
    ...
    d_rpc_fault(B);
d_rpc_response(<<_RPC_VERSION, _RPC_MINOR_VERSION, OP, _/binary>>) ->
...
%%%
d_rpc_bind_ack(B0) ->
    <<_PacketFlags,                 % 4:1 (byte:len)
      _DataRepresentation:32/little,
      _FragLength:16/little,
      _AuthLength:16/little,
      _Call_ID:32/little,            % 12:4
      _MaxXmitFrag:16/little,
      _MaxRecvFrag:16/little,
      AssocGroup:32/little,         % 20:4 
      ScndryAddrLen:16/little,      % 24:2 
      B1/binary>>  = B0,
    %% Restore 4-octet alignment
    Pad = 4 - ((26 + ScndryAddrLen) rem 4),
    <<_ScndryAddr:ScndryAddrLen/binary, % null term.ASCII string
      _:Pad/?BYTE,
      B2/binary>> = B1,
    ...
    <<AckRes:16/little,  
      ...
      _TransferSyntax:16/binary,
      _SyntaxVersion:32/little,
...
d_rpc_bind_nack(B0) ->
    <<_PacketFlags,
      ...
      _NumOfProtos,
      _B1/binary>> = B0, % list(_NumOfProtos): <<Major:8,Minor:8,...>>
