...
%% This is the original crash report by Peter Wang (10 July 2007) triggering an
%% R11B-5 crash: strength reduction could not handle calls with no destination
crash_0(A) ->
  case A of
    0 ->
      A div 8,
...
%% The above was simplified to the following which showed another crash,
%% this time on RTL
crash_1(A) when is_integer(A), A >= 0 ->
  A div 8,
  ok. 

%% A similar crash like the first one, but in a different place in the code,
